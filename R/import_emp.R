...
import_emp <- function(folder) {
  ...
  for(i in 1:length(input_names)) {
    ...
    emp_el_w[[i]] <- cbind(unique(get.edgelist(emp_g[[i]])), weight = NA, n_nodes = c(vcount(emp_g[[i]]), rep(NA, ecount(emp_g[[i]]) / 2 - 1)), n_edges = c(ecount(emp_g[[i]]) / 2, rep(NA, ecount(emp_g[[i]]) / 2 - 1)))
