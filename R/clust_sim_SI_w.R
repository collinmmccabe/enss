#' clust_sim_SI_w
#'
#' clust_sim_SI_w simulates the spread of an infectious disease on a weighted
#' social network using the Susceptible-Infected compartmental model in parallel
...
clust_sim_SI_w <- function(network_el, beta, intxn_per_day, days, iters, free_threads = 1) {
  ...
  parallel::clusterExport(cl = cl, varlist = c("network_el", "sim_SI_w", "beta", "intxn_per_day", "days", "iters"), envir = environment())

  sim_SI_w.out <- pbapply::pblapply(c(1:length(network_el)), function(x,b,i,d,it) { print(replicate(it, (sim_SI_w(network_el[[x]],b,i,d)))) }, b = beta, i = intxn_per_day, d = days, it = iters, cl = cl)

  parallel::stopCluster(cl); return(sim_SI_w.out)
