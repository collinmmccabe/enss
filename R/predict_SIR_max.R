...
#'
#' @param sim_max_output simulation output from any of the sim_SIR functions (including
...
#'
#' @examples
predict_SIR_max <- function(sim_max_output, n_min) {

  SIR_max_output <- data.frame(days = NA, peak = NA, n = NA)

  for(i in 1:length(sim_max_output)) {

    tmp <- data.frame(days = sim_max_output[[i]][1,which(sim_max_output[[i]][4,] == i)], peak = sim_max_output[[i]][5,which(sim_max_output[[i]][4,] == i)], n = n_min + i - 1)
