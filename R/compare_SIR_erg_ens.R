#' compare_SIR_erg_ens
#'
#' compare_SIR_erg_ens calculates multiple metrics of comparison between maximally
#'   complete networks and Erdos-Renyi random graphs for clustered SIR simulations
#'
#' @param sim_erg_output output of SIR simulations over all Erdos-Renyi random graphs,
...
#' @param sim_max_output output of SIR simulations over all maximally complete graphs
...
compare_SIR_erg_ens <- function(sim_erg_output, sim_max_output, n_min) {
  ...
  predict_SIR_output <- predict_SIR_max(sim_max_output, n_min)
  ...
  for (n in nodes) {
    ...
    for (p in percents) {
      ...
      for(i in 1:samples) {
        ...
        tmp.exp <- sim_max_output[[estimate_backtrans_ens(mean(tmp.obs), predict_SIR_output, sim_max_output) - n_min + 1]][1,]
