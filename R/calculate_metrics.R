#' calculate_metrics
#'
#' calculate_metrics calculates for each edgelist in a list of edgelists the following
...
calculate_metrics <- function(emp_el) {
  ...
  for (y in 1:length(emp_el)) {
    ...
    ig <- igraph::graph.data.frame(b, directed = FALSE)
    dens <- igraph::graph.density(ig) / 2
    cent <- igraph::evcent(ig, weights = w, scale = FALSE)
    Ce.ei <- (-sum(cent$vector - max(cent$vector))) / sum(rep(1, cent$options$n) - cent$vector)
    Newman.mod <- igraph::leading.eigenvector.community(ig, weights = w)$modularity
    diam <- igraph::diameter(ig, directed = FALSE, weights = w)
    size <- cent$options$n

    c <- tnet::symmetrise_w(b)
    Cl.wgm <- tnet::clustering_w(c, measure = "gm")
    dist.mat <- tnet::distance_w(c)
