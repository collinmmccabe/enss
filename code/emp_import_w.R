...
for(i in 1:length(input_names)) {
  ...
  emp_el[[i]] <- cbind(unique(get.edgelist(emp_g[[i]])), weight=as.numeric(tmp[,3]), n_nodes=c(vcount(emp_g[[i]]), rep(NA, ecount(emp_g[[i]])/2-1)), n_edges=c(ecount(emp_g[[i]])/2, rep(NA, ecount(emp_g[[i]])/2-1)))
...
saveRDS(emp_g,file="./sims_empirical/Effective_g_emp_w")
saveRDS(emp_el,file="./sims_empirical/Effective_el_emp_w")
saveRDS(spp_list,file="./sims_empirical/Effective_spp_emp_w")
